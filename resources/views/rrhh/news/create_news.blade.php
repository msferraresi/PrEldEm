                ...
                <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        ...
                        <div class="mt-5 md:mt-0 md:col-span-2">
                            <form method="POST" action="{{ route('rrhh.store_news') }}">
                                @csrf
                                <div class="shadow overflow-hidden sm:rounded-md">
                                    <div class="block mb-8">
                                        ...
                                        <div class="px-4 py-5 bg-white sm:p-6">
                                            ...
                                            <textarea name="description" id="description"  rows="4" cols="50" class="form-input rounded-md shadow-sm mt-1 block w-full">{{ old('description', '') }}</textarea>
                                            @error('description')
                                                <p class="text-sm text-red-600">{{ $message }}</p>
                                            @enderror
                                        </div>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="px-6 py-3 whitespace-nowrap text-sm font-small">
                                                            <div class="form-check">
                                                                <input class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 my-1 align-top bg-no-repeat bg-center bg-contain float-left cursor-pointer" type="checkbox" value="{{$areas[$i]->id}}" id="{{'chk'.$areas[$i]->id}}" name="{{'chk'.$areas[$i]->id}}">
                                                            </div>
                                                        </td>
                                                    </tr>
                                                @endfor
